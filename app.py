...
st.subheader("✅ Respuestas correctas por ronda")
respuestas_correctas = []
for i in range(num_rondas):
    respuestas = st.text_area(f"Ronda {i+1} (separa por espacios o líneas)", height=100)
    respuestas_correctas.append(re.findall(r'\S+', respuestas))

# Texto completo de la dinámica
st.subheader("📄 Texto completo de la dinámica")
texto_dinamica = st.text_area("Pega aquí todo el texto", height=500)

if st.button("🔍 Analizar participación"):
    # Dividir mensajes por timestamps
    ...
    for idx_ronda in range(num_rondas):
        ...
        for mensaje in mensajes:
            ...
            if not mensaje:
                continue
            if any(r in mensaje for r in respuestas) and any(e in mensaje for e in emojis_casa):
